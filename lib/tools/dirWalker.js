...
function walk(uri,filter,files) {
	...
	if(filter(uri)){
		if(stat.isFile()){
			...
			if([widget].every(function(item,i){
				return uri.indexOf(item) === -1;
			})){//如果是page的话
				if(uri.indexOf(pagelet) === -1){
					switch(path.extname(uri)){
						case '.js':
							//js目录下仅保留i18n子目录下js
							if(uri.indexOf(js) !== -1){
								if(uri.indexOf(path.join(js,'i18n')) !== -1 ||uri.indexOf(path.join(js,'lib')) !== -1 ){
									files.js.push(uri);
								}
							}else{
								files.js.push(uri);
				...
				}else if(uri.indexOf(path.join(pagelet,'public_')) === 0){//pagelet
					switch(path.extname(uri)){
						case '.js':
							//js目录下仅保留i18n和lib子目录下js
							if(uri.indexOf(js) !== -1){
								if(uri.indexOf(path.join(js,'i18n')) !== -1 || uri.indexOf(path.join(js,'lib')) !== -1 ){
									files.js.push(uri);
								}
